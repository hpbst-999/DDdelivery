...
from src.identity.domain.entities import Account
...
from src.identity.infrastructure.postgres_repositories import SQLAlchemyAccountRepository
...
def get_request_otp_use_case(
    redis_uow: IUnitOfWork = Depends(get_redis_uow),
    sms_gateway: SmsSender = Depends(get_sms_sender)
) -> RequestOTPUseCase:
    return RequestOTPUseCase(redis_uow=redis_uow, sms_gateway=sms_gateway)

def get_refresh_session_use_case(
    redis_uow: IUnitOfWork = Depends(get_redis_uow),
    token_service: JwtTokenService = Depends(get_token_service)
) -> RefreshSessionUseCase:
    return RefreshSessionUseCase(redis_uow=redis_uow, token_service=token_service)

def get_logout_use_case(
    redis_uow: IUnitOfWork = Depends(get_redis_uow)
) -> LogoutUseCase:
    return LogoutUseCase(redis_uow=redis_uow)

def get_verify_courier_otp_use_case(
    redis_uow: IUnitOfWork = Depends(get_redis_uow),
    pg_uow: IUnitOfWork = Depends(get_pg_uow),
    token_service: JwtTokenService = Depends(get_token_service)
...
def get_update_courier_profile_use_case(
    ...
    return UpdateCourierProfileUseCase(pg_uow=pg_uow)

def get_update_user_profile_use_case(
    pg_uow: IUnitOfWork = Depends(get_pg_uow)
) -> UpdateUserProfileUseCase:
    return UpdateUserProfileUseCase(pg_uow=pg_uow)

def get_delete_courier_use_case(
    pg_uow: IUnitOfWork = Depends(get_pg_uow)
) -> DeleteCourierUseCase:
    return DeleteCourierUseCase(pg_uow=pg_uow)

def get_delete_user_use_case(
    pg_uow: IUnitOfWork = Depends(get_pg_uow)
) -> DeleteUserUseCase:
    return DeleteUserUseCase(pg_uow=pg_uow)

def get_courier_profile_use_case(
    pg_uow: IUnitOfWork = Depends(get_pg_uow)
) -> GetCourierProfileUseCase:
    return GetCourierProfileUseCase(pg_uow=pg_uow)

def get_user_profile_use_case(
    pg_uow: IUnitOfWork = Depends(get_pg_uow)
) -> GetUserProfileUseCase:
    return GetUserProfileUseCase(pg_uow=pg_uow)
...
def get_current_account(
    ...
    with pg_uow:
        account = pg_uow.accounts.get_account_by_id(account_id)
        if not account:
